...
type ConfidentialCampaignRow = {
  campaign: string;
  status: "Before" | "Variant" | "Winner";
  delivery: string;
  action: string;
  bidStrategy: string;
  budget: string;
...
const CONFIDENTIAL_CAMPAIGN_ROWS: ConfidentialCampaignRow[] = [
  {
    campaign: "Control landing page",
    status: "Before",
    delivery: "Paused",
    action: "—",
    ...
    results: "2,847",
    reach: "473,420",
    impressions: "1,289,650",
    conversionRate: "6.00%",
    costPerResult: "$4.20",
    amountSpent: "$11,957.40",
  },
  {
    campaign: "Variant A: meetup framing",
    status: "Variant",
    delivery: "Active",
    ...
    reach: "284,710",
    impressions: "776,420",
    conversionRate: "17.30%",
    costPerResult: "$1.74",
    costDelta: "-59%",
    amountSpent: "$8,560.80",
  },
  {
    campaign: "Variant B: community-led framing",
    status: "Winner",
    delivery: "Active",
    ...
    costPerResult: "$0.85",
    costDelta: "-80%",
    amountSpent: "$6,050.30",
  },
];

const CONFIDENTIAL_CAMPAIGN_TOTALS = {
  results: "14,885",
  reach: "1,042,950",
  impressions: "2,845,280",
  avgCostPerPurchase: "$1.79",
  amountSpent: "$26,568.50",
};
    ...
    <main className="bg-white text-[#3c3e3f] overflow-x-hidden">
      ...
      {caseStudy.slug === CONFIDENTIAL_PLACEHOLDER_SLUG ? (
        <section className="mx-auto max-w-[1200px] px-6 py-10 md:px-10 xl:px-20">
          ...
          <p className="mx-auto mb-10 max-w-[760px] text-center font-inter text-[16px] leading-[1.7] text-[#5c7792]">
            The experiment compared the original control landing page against two new variants under the same budget conditions. Client naming has been anonymized, but the relative performance patterns are preserved.
          </p>
          ...
          <div className="grid gap-4 md:hidden">
            {CONFIDENTIAL_CAMPAIGN_ROWS.map((row) => (
              <Card key={row.campaign} className={`overflow-hidden ${row.status === "Winner" ? "border-[#9cd99f] bg-[#f0fdf4]" : ""}`}>
                <CardContent className="space-y-4 px-5 py-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-[18px] font-semibold leading-snug text-[#0e2951]">{row.campaign}</p>
                      <p className="mt-2 text-[14px] text-[#5c7792]">{row.delivery}</p>
                    </div>
                    <Badge className={row.status === "Winner" ? "" : row.status === "Before" ? "border border-[#65676b] bg-white text-[#65676b]" : "border border-[#1183D0] bg-white text-[#1183D0]"}>
                      {row.status}
                    </Badge>
                  </div>
                  <div className="grid grid-cols-2 gap-4 text-[14px] leading-[1.6] text-[#5c7792]">
                    <div>
                      <p className="text-[12px] font-semibold uppercase tracking-[0.2em] text-[#1183D0]">Results</p>
                      <p className="mt-1 font-semibold text-[#0e2951]">{row.results} {row.resultsDelta ?? ""}</p>
                    </div>
                    <div>
                      <p className="text-[12px] font-semibold uppercase tracking-[0.2em] text-[#1183D0]">Conversion</p>
                      <p className="mt-1 font-semibold text-[#0e2951]">{row.conversionRate}</p>
                    </div>
                    <div>
                      <p className="text-[12px] font-semibold uppercase tracking-[0.2em] text-[#1183D0]">Cost / purchase</p>
                      <p className="mt-1 font-semibold text-[#0e2951]">{row.costPerResult} {row.costDelta ?? ""}</p>
                    </div>
                    <div>
                      <p className="text-[12px] font-semibold uppercase tracking-[0.2em] text-[#1183D0]">Spend</p>
                      <p className="mt-1 font-semibold text-[#0e2951]">{row.amountSpent}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
