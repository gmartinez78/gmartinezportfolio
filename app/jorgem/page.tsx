  ...
  return <div style={{ minHeight: "100vh", background: colors.blueSoft, color: colors.navy, fontFamily: "Arial, sans-serif" }}>
    ...
    <main style={{ maxWidth: 760, margin: "auto", padding: "20px 14px 56px", display: "grid", gap: 18 }}>
      ...
      {activeDay && <><Card><div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start" }}><div><h1 className="jorgem-title" style={{ margin: 0, fontSize: 23 }}>{activeDay.label} · {activeDay.title}</h1><p style={{ margin: "4px 0 0", color: colors.text }}>{activeDay.subtitle}</p></div><div style={{ display: "grid", gap: 6, justifyItems: "end" }}><label style={{ color: colors.text, fontSize: 11, fontWeight: 800 }}>SEMANA<select value={selectedWeek} onChange={(event) => setSelectedWeek(Number(event.target.value))} style={{ display: "block", marginTop: 3, border: `1px solid ${colors.border}`, borderRadius: 8, padding: "6px 8px", color: colors.navy, background: colors.white, font: "inherit", fontSize: 12 }}>{Array.from({ length: 8 }, (_, index) => <option key={index + 1} value={index + 1}>Semana {index + 1}</option>)}</select></label><Pill tone="success">{activeDay.exercises.filter((_, index) => done[`${activeDay.key}-${index}`]).length}/{activeDay.exercises.length} hecho</Pill></div></div><div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 14 }}><Pill>RPE 6-8</Pill><Pill>No aguantes la respiración</Pill><Pill tone="warning">Codo: máximo 3/10</Pill></div></Card>
        ...
        <Card><h2 style={sectionTitle}><Dumbbell size={19} /> Ejercicios</h2><div style={{ display: "grid", gap: 10 }}>{activeDay.exercises.map(([name, sets, rest, pairing], index) => { const id = `${activeDay.key}-${index}`; const checked = Boolean(done[id]); const metrics = exerciseMetrics[id] ?? { weight: "", reps: "" }; return <article className="jorgem-card" key={id} style={{ background: checked ? colors.successBg : colors.blueSoft, border: `1px solid ${checked ? colors.success : colors.border}`, borderRadius: 14, padding: compact ? 11 : 15 }}><div style={{ display: "grid", gridTemplateColumns: compact ? "1fr" : "minmax(112px, 30%) 1fr", gap: 12 }}><ExerciseImage dayKey={activeDay.key} index={index} name={name} /><div><div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><div><strong>{index + 1}. {name}</strong><div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginTop: 8 }}><Pill>{sets}</Pill><Pill><Clock size={13} /> Descanso: {rest}</Pill></div></div><button onClick={() => void saveExercise(activeDay.key, index, { completed: !checked })} aria-pressed={checked} style={{ alignSelf: "start", display: "inline-flex", alignItems: "center", gap: 5, border: `1px solid ${checked ? colors.success : colors.border}`, borderRadius: 9, background: colors.white, color: checked ? colors.success : colors.navy, padding: "8px 10px", cursor: "pointer", fontWeight: 800, fontSize: 12 }}><CheckCircle2 size={15} />{checked ? "Hecho" : "Marcar"}</button></div>{pairing ? <p style={{ margin: "10px 0 0", fontSize: 13, lineHeight: 1.5, color: colors.navy }}><strong>{pairing.startsWith("Biserie") ? "Conexión: " : "Nota: "}</strong>{pairing}</p> : null}{!compact && <><div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}><label style={{ color: colors.text, fontSize: 12, fontWeight: 700 }}>Peso (kg)<input value={metrics.weight} onChange={(event) => void saveExercise(activeDay.key, index, { weight: event.target.value })} inputMode="decimal" style={metricInput} /></label><label style={{ color: colors.text, fontSize: 12, fontWeight: 700 }}>Repeticiones<input value={metrics.reps} onChange={(event) => void saveExercise(activeDay.key, index, { reps: event.target.value })} inputMode="numeric" style={metricInput} /></label></div><p style={{ margin: "11px 0 0", fontSize: 13, color: colors.text }}>Prioriza una técnica controlada y detén el ejercicio si la molestia aumenta.</p></>}</div></div></article>; })}</div></Card>
