...
type Role = "assistant" | "patient" | "clinician";
...
export function CalendarKeeperMobilePrototype() {
  ...
  const addAppointment = () => {
    ...
    setScreen(role === "patient" ? "role" : "schedule");
  };
      ...
      <div className="overflow-hidden rounded-[32px] bg-[#f4f8ff]">
        ...
        <div className="min-h-[650px] p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-[10px] font-bold tracking-[0.15em] text-[#2863d8]">CLARITY SCHEDULE</p><h3 className="mt-1 text-[18px] font-bold text-[#24324a]">{screen === "role" ? "Choose a role" : screen === "book" ? role === "patient" ? "Book a return visit" : "New appointment" : screen === "queue" ? "Action queue" : screen === "setup" ? "First bookable schedule" : role === "clinician" ? "Today’s clinic" : "Good morning, Sofia."}</h3></div>
            {screen !== "role" ? <button type="button" onClick={() => { setIsDegraded((current) => !current); setNotice(isDegraded ? "Integration restored. Review outstanding appointments." : "Degraded mode is active. Scheduling remains available."); }} className={`rounded-full px-2.5 py-1.5 text-[10px] font-bold ${isDegraded ? "bg-[#fff3d8] text-[#895400]" : "bg-[#e7f6ee] text-[#19714e]"}`}>{isDegraded ? "Not synced" : "Synced"}</button> : null}
          </div>

          {screen === "role" ? <div className="mt-8"><p className="text-[12px] leading-[1.5] text-[#667085]">Select the perspective you want to explore in this prototype.</p><div className="mt-5 space-y-3">{[{ id: "patient", label: "Returning patient", description: "Book or manage a familiar visit" }, { id: "assistant", label: "Medical assistant", description: "Coordinate the daily schedule" }, { id: "clinician", label: "Clinician", description: "Review capacity and appointments" }].map((item) => <button key={item.id} type="button" onClick={() => setRole(item.id as Role)} className={`w-full rounded-2xl border p-4 text-left ${role === item.id ? "border-[#2863d8] bg-[#eaf1fc] shadow-[0_8px_20px_rgba(40,99,216,0.08)]" : "border-[#d8e2f1] bg-white"}`}><span className="block text-[12px] font-bold text-[#24324a]">{item.label}</span><span className="mt-1 block text-[10px] text-[#667085]">{item.description}</span></button>)}</div><button type="button" onClick={() => setScreen(role === "patient" ? "book" : "schedule")} className="mt-5 w-full rounded-xl bg-[#2863d8] px-4 py-3 text-[12px] font-bold text-white">Explore as {role === "patient" ? "a patient" : role === "assistant" ? "an assistant" : "a clinician"}</button></div> : null}
        ...
        {screen !== "role" ? <nav className="flex border-t border-[#d8e2f1] bg-white px-3 py-2" aria-label="Prototype navigation">
          {(role === "patient" ? [{ id: "book", label: "Book" }, { id: "role", label: "Switch" }] : [{ id: "schedule", label: "Schedule" }, { id: "book", label: "New" }, { id: "queue", label: "Queue" }, { id: "setup", label: "Setup" }, { id: "role", label: "Switch" }]).map((item) => <button key={item.id} type="button" onClick={() => setScreen(item.id as Screen)} className={`flex-1 rounded-lg px-2 py-2 text-[10px] font-bold ${screen === item.id ? "bg-[#2863d8] text-white" : "text-[#667085]"}`}>{item.label}</button>)}
        </nav> : null}
      </div>
    </div>
  );
