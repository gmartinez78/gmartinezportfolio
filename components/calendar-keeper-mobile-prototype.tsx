...
type Screen = "schedule" | "book" | "queue";
      ...
      <div className="overflow-hidden rounded-[32px] bg-[#f4f8ff]">
        ...
        <div className="min-h-[650px] p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-[10px] font-bold tracking-[0.15em] text-[#2863d8]">CLARITY SCHEDULE</p><h3 className="mt-1 text-[18px] font-bold text-[#24324a]">{screen === "book" ? "New appointment" : screen === "queue" ? "Action queue" : "Good morning, Sofia."}</h3></div>
        ...
        <nav className="flex border-t border-[#d8e2f1] bg-white px-3 py-2" aria-label="Prototype navigation">
          {[{ id: "schedule", label: "Schedule" }, { id: "book", label: "New" }, { id: "queue", label: "Queue" }].map((item) => <button key={item.id} type="button" onClick={() => setScreen(item.id as Screen)} className={`flex-1 rounded-lg px-2 py-2 text-[10px] font-bold ${screen === item.id ? "bg-[#2863d8] text-white" : "text-[#667085]"}`}>{item.label}</button>)}
        </nav>
      </div>
    </div>
  );
