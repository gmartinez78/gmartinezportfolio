...
const slides = [
  ...
  { eyebrow: "The recommendation", title: "Launch the most dependable service first.", body: "Start with a schedule staff can trust—one that understands every appointment and keeps working when systems fail. Backfill comes later, when there’s enough demand." },
  ...
  { eyebrow: "What I would validate next", title: "Prove that the first release earns operational trust.", body: "Validating recovery, coordination, and self-service before scaling." },
];
        ...
        <div className="w-full">
          ...
          <div key={activeSlide} className="animate-in fade-in slide-in-from-right-3 duration-300">
            {activeSlide === 0 ? (
              ...
              <div className="rounded-[32px] bg-[#0e2951] px-7 py-14 text-white md:px-16 md:py-20"><h1 className="max-w-[870px] font-inter text-5xl font-semibold leading-[1.07] tracking-[-0.045em] md:text-6xl">{slide.title}</h1><p className="mt-7 max-w-[720px] text-[19px] leading-relaxed text-[#d4e5f2]">{slide.body}</p><div className="mt-12 grid gap-3 md:grid-cols-2">{[["Can staff resolve a sync exception without training?"], ["Do resource rules prevent rework before booking?"], ["Can returning patients complete routine changes without calling?"], ["Does the experience scale from a team practice to a solo practitioner?", "France"]].map(([item, market]) => <p key={item} className="flex items-start gap-3 rounded-xl border border-white/15 px-4 py-4 text-[15px] text-[#d4e5f2]"><Check className="mt-0.5 h-4 w-4 shrink-0 text-[#83cef8]" /><span>{item}</span>{market ? <span className="ml-auto shrink-0 rounded-full bg-[#83cef8] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#0e2951]">{market}</span> : null}</p>)}</div></div>
            )}
          </div>
        </div>
      </section>
