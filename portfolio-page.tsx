...
export default function PortfolioPage() {
  ...
  const impactSection = activeImpactProject ? (
    <section key="impact" className="bg-white px-6 py-16 md:px-10 xl:px-20">
      <div className="mx-auto flex w-full max-w-[1240px] flex-col gap-10 rounded-[38px] border border-white/8 bg-[#0a0a0d] p-8 text-white shadow-[0_24px_70px_rgba(0,0,0,0.28)] md:p-12">
        <div className="max-w-[980px]">
          <p className="text-[13px] font-semibold uppercase tracking-[0.28em] text-[#7CB8E8]">
            Recruiter Snapshot
          </p>
          <h2 className="mt-5 max-w-[860px] font-serif-display text-[42px] leading-[0.98] text-white md:text-[72px]">
            High-impact product work, with proof in the flow.
          </h2>
          <p className="mt-6 max-w-[980px] text-[18px] leading-[1.7] text-white/62 md:text-[20px]">
            A quick view into the projects with the strongest measurable outcomes, plus how I now use fast AI-assisted
            iteration with Codex to move from working code to polished product decisions without losing UX clarity.
          </p>
        </div>

        <div className="inline-flex w-full max-w-[980px] flex-wrap gap-2 rounded-[24px] border border-white/8 bg-white/6 p-2">
            ...
            <button
              ...
              className={`rounded-[18px] px-4 py-3 text-left transition-all ${
                index === activeImpactIndex
                  ? "bg-[#17171d] text-white shadow-[inset_0_0_0_1px_rgba(255,255,255,0.08)]"
                  : "text-white/62 hover:bg-white/5 hover:text-white"
              }`}
            >
              <span className="block text-[12px] font-semibold uppercase tracking-[0.22em] text-[#7CB8E8]">
                {project.label}
              </span>
              <span className="mt-1 block text-[18px] font-semibold">{project.title}</span>
            </button>
          ))}
        </div>

        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[30px] border border-white/8 bg-[#17171d] p-8">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-[13px] text-white/48">
              <span>{activeImpactProject.company}</span>
              <span className="text-white/18">•</span>
              <span>{activeImpactProject.role}</span>
              <span className="text-white/18">•</span>
              <span>{activeImpactProject.year}</span>
            </div>
            <h3 className="mt-4 text-[34px] font-semibold leading-tight text-white">
              {activeImpactProject.title}
            </h3>
            <p className="mt-4 text-[17px] leading-[1.75] text-white/68">
              {activeImpactProject.tagline}
            </p>
            <div className="mt-7 grid gap-4 sm:grid-cols-2">
              <div className="rounded-[22px] border border-white/8 bg-white/4 p-5">
                <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#7CB8E8]">Top signal</p>
                <p className="mt-3 text-[34px] font-semibold text-white">{activeImpactProject.metricValue}</p>
                <p className="mt-2 text-[14px] leading-relaxed text-white/58">{activeImpactProject.metricLabel}</p>
              </div>
              <div className="rounded-[22px] border border-white/8 bg-white/4 p-5">
                <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#7CB8E8]">Why recruiters care</p>
                <p className="mt-3 text-[15px] leading-relaxed text-white/68">{activeImpactProject.summary}</p>
              </div>
            </div>
            <div className="mt-7 space-y-3">
              {activeImpactProject.recruiterBullets.map((bullet) => (
                <div key={bullet} className="flex gap-3 rounded-[18px] border border-white/6 bg-white/4 px-4 py-4">
                  <span className="mt-1 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#1183D0]/18 text-[12px] font-semibold text-[#8FD3FF]">
                    ✓
                  </span>
                  <p className="text-[15px] leading-[1.7] text-white/72">{bullet}</p>
                </div>
              ))}
            </div>
            <div className="mt-8">
              <Link
                href={activeImpactProject.href}
                className="inline-flex items-center rounded-[16px] bg-white px-5 py-3 text-[15px] font-semibold text-[#0a0a0d] transition-transform hover:-translate-y-0.5"
              >
                View case study
              </Link>
            </div>
          ...
          <div className="rounded-[30px] border border-white/8 bg-[#17171d] p-6">
            <div className="rounded-[22px] border border-white/8 bg-[#111216] p-4">
              <div className="flex items-center justify-between border-b border-white/8 pb-4">
                <div className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full bg-[#ff7d7d]" />
                  <span className="h-2.5 w-2.5 rounded-full bg-[#ffca66]" />
                  <span className="h-2.5 w-2.5 rounded-full bg-[#6fdd8b]" />
                </div>
                <div className="rounded-full border border-[#2c3442] bg-[#0d1017] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#8FD3FF]">
                  Codex workflow
                </div>
              </div>

              <div className="mt-5 grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
                <div className="rounded-[20px] border border-white/8 bg-[#0d1017] p-4">
                  <div className="mb-4 flex items-center justify-between">
                    <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-white/42">Code</p>
                    <span className="text-[12px] text-white/38">portfolio/{activeImpactProject.slug}.tsx</span>
                  </div>
                  <div className="space-y-3 font-mono text-[12px] leading-6 text-[#d7e8ff]">
                    {activeImpactProject.codeLines.map((line, index) => (
                      <div
                        key={`${activeImpactProject.slug}-${index}`}
                        className="flex gap-3 rounded-[14px] px-3 py-2 transition-all duration-500"
                        style={{
                          background: index === 1 ? "rgba(17,131,208,0.12)" : "rgba(255,255,255,0.02)",
                ...
                <div className="rounded-[20px] border border-white/8 bg-[#0d1017] p-4">
                  <div className="mb-4 flex items-center justify-between">
                    <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-white/42">Preview</p>
                    <span className="rounded-full bg-[#1183D0]/16 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#8FD3FF]">
                      Live refresh
                    </span>
                  </div>
                  <div className="relative overflow-hidden rounded-[18px] border border-white/8 bg-[radial-gradient(circle_at_top,#1b3554_0%,#101722_58%,#0a0a0d_100%)] p-5">
                    <div className="pointer-events-none absolute inset-x-6 top-6 h-24 rounded-[20px] bg-[linear-gradient(135deg,rgba(17,131,208,0.22),rgba(143,211,255,0.05))] blur-xl" />
                    <div className="relative rounded-[18px] border border-white/10 bg-[#121822]/86 p-4 shadow-[0_12px_34px_rgba(0,0,0,0.28)] transition-all duration-500">
                      <div className="flex items-center justify-between">
                        <span className="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#8FD3FF]">
                          {activeImpactProject.label}
                        </span>
                        <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] text-white/56">
                          Recruiter mode
                        </span>
                      </div>
                      <h4 className="mt-4 text-[24px] font-semibold leading-tight text-white">
                        {activeImpactProject.previewTitle}
                      </h4>
                      <div className="mt-4 flex flex-wrap gap-2">
                        {activeImpactProject.previewNotes.map((note) => (
                          <span
                            key={note}
                            className="rounded-full border border-white/10 bg-white/6 px-3 py-1.5 text-[12px] font-medium text-white/72 transition-all duration-500"
                          >
                            {note}
                          </span>
                        ))}
                      ...
                      <div className="mt-6 grid gap-3">
                        <div className="h-3 w-[72%] rounded-full bg-white/14" />
                        <div className="h-3 w-[48%] rounded-full bg-[#1183D0]/40" />
                        <div className="grid grid-cols-[1.2fr_0.8fr] gap-3">
                          <div className="h-32 rounded-[18px] bg-[linear-gradient(180deg,rgba(143,211,255,0.18),rgba(17,131,208,0.08))]" />
                          <div className="space-y-3">
                            <div className="h-14 rounded-[16px] bg-white/8" />
                            <div className="h-14 rounded-[16px] bg-white/6" />
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <p className="mt-4 text-[13px] leading-relaxed text-white/48">
                    Fast iteration loop: prompt, ship code, review the UI, refine the product decision, and hand off cleaner intent.
                  </p>
                </div>
              </div>
            </div>
  ...
  const contentSections = [impactSection, toolsSection, ctaSection];
